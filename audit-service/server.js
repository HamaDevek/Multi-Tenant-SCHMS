...
const storeAuditLog = async (auditData) => {
  const { userId, tenantId, action, status, ipAddress, userAgent, details } = auditData;
  const logId = uuid.v4();

  try {
    if (!tenantId) {
      // Store in master database for super admin actions
      ...
      try {
        ...
        // Fallback to master database if tenant database is not available
        await masterPool.query(
          `INSERT INTO audit_logs (id, userId, action, status, ipAddress, userAgent, details)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [logId, userId, action, status, ipAddress, userAgent, 
...
app.get('/failed-logins', async (req, res) => {
  try {
    ...
    // Get master database failed logins (super admin)
    ...
    allFailedLogins.push(...masterLogs.map(log => ({
      ...log,
      tenantName: 'Master',
      tenantId: null
    })));

    // Get failed logins for each tenant
    for (const tenant of tenants) {
      try {
        ...
        console.error(`Error getting failed logins for tenant ${tenant.id}:`, error);
      }
    }
...
app.get('/:tenantId/failed-logins', async (req, res) => {
  try {
    ...
    console.error(`Get failed logins for tenant ${req.params.tenantId} error:`, error);
    res.status(500).json({
      status: 'error',
      message: 'An error occurred while fetching failed logins'
    });
...
app.get('/:tenantId/logs', async (req, res) => {
  try {
    const { tenantId } = req.params;
    const { action, status, startDate, endDate, limit = 100, page = 1 } = req.query;
    ...
    console.error(`Get audit logs for tenant ${req.params.tenantId} error:`, error);
    res.status(500).json({
      status: 'error',
